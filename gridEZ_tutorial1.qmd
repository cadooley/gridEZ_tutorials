...
```{r}
# read in from file after download
...
Next, we need to convert the sf object containing administrative units into a raster. We do this using the grid of the population raster. The `field` argument to the `rasterize` function should be a variable in admin_sf that uniquely identifies the administrative units; we'll use GID_2 in this example.
...
df <- data.frame(
  ...
  c("Water", "Mostly uninhabited", "Dispersed rural", "Village", 
    "Suburban or peri-urban", "Semi-dense town", "Dense town", "City"),
  c("NA", "Rural", "Rural", "Rural", 
...
-   .tif (raster) with file name beginning "EZ_IDs\_". Raster of the sampling frame. Each grid cell shows the ID of the EZ it belongs to.
...
EZ_tab <- read.csv(paste0(file_path_outputs, "EZ_Pop_Ncells_wsm_large.csv"), header = TRUE)
...
```{r}
# read in EZs table of your sampling frame 
EZ_tab_ownspecs <- read.csv(paste0(file_path_outputs, "EZ_Pop_Ncells_wsm_large_ownspecs.csv"), header = TRUE)
# number of EZs in your sampling frame
...
From this we can see that the same sampling frame has been made. The goal of creating the two sampling frames was to demonstrate the two different approaches. Now you have everything you need to start creating sampling frames using a variety of target EZ sizes.
...
ggplot(res_poly, aes(x = N, y = pop)) +
  ...
  # grey scatter plot area for values within acceptable ranges
  annotate("rect",
           xmin = 0, xmax = 3750,
           ymin = 800, ymax = 1600,
