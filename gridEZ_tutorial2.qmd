...
required_packages <- c("sf", "terra", "parallel", "data.table", "ggplot2", "scico")
...
We will use two different population datasets for creating sampling frames in this tutorial: GHS-POP and WorldPop (which we also used in Tutorial 1).
...
If you are interested in creating sampling frames for areas constrained to settlements, please check out Tutorial 4.
...
thresholds_tab <- data.table::data.table(
  popdata = rep(c("wp", "ghs"), each = 3), 
  EZsize = rep(c("small", "med", "large"), 2), 
  minpop = rep(c(50, 333, 800), 2), 
  maxpop = rep(c(100, 667, 1600), 2), 
  lowermaxgeog = rep(c(75, 675, 1875), 2), 
  uppermaxgeog = rep(c(150, 1350, 3750), 2))
thresholds_tab
```
...
res_poly_list <- lapply(split(thresholds_tab, seq_len(nrow(thresholds_tab))), function(x){
  ...
  EZ_tab <- read.csv(paste(file_name), header = TRUE)
...
You can see that EZs across the two sampling frame show a number of similarities in terms of size, shape and category for many locations. The main obvious difference is the angle of many EZ boundaries which comes from the CRS of the administrative boundaries differing.
