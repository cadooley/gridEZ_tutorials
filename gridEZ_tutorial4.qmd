---
title: "Tutorial 4: applying gridEZ to population constrained to settled areas"
author: 
  - name: "Claire Dooley"
...
format: html
editor: visual
suggested-citation: "Dooley, C.A. (2025). gridEZ Tutorial 4 version 1.0"
...
The aim of this tutorial is to generate and evaluate a bespoke gridded sampling frame constrained to settled areas across Samoa. For this example we stratify by administrative units only and demonstrate how to run gridEZ with justone strata dataset rather than two. The gridded sampling frame will contain units called gridded Enumeration Zones (EZs) that can be sampled from for surveys.

Surveys may benefit from sampling frames constrained to settled areas, as field teams need to locate households efficiently. Uncertain or sparsely settled areas increase travel time and costs and raise the risk of finding no eligible households, potentially disrupting fieldwork schedules and wasting resources.

At the same time, restricting the sampling frame to settled areas requires careful consideration. Small or transient settlements may be absent from the available population data used to delineate the coverage of the frame. Therefore, when the sampling frame is limited to areas identified as settled, any locations outside those areas—including settlements that are erroneously missed—have zero probability of being sampled; by contrast, a sampling frame covering the entire region may include inaccurate population estimates for some units/EZs, but all areas retain a non-zero probability of selection.
...
We'll use the WorldPop population data [@bondarenko2025] and the GADM administraive boundare [@GADM2022] that were used in the previous tutorials. More information about the WorldPop population data can be found in Tutorial 1. In this tutorial we will take advantage of the counts being constrained to settled areas only.

Datasets will be read in from a folder called "data_wsm"*.* The following code checks whether the folder exists and creates it if it’s not found.
